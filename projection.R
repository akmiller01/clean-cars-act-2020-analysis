...
zev_p.ts = ts(rep(NA,60),frequency=12,start=c(2020,1))
...
zev_posterior.interval <- cbind.data.frame(
  ...
  subset(zev_d2, Date>as.Date("2019-12-01"))$Date)
...
sum.p = ggplot(data=d4, aes(x=Date)) +
  geom_line(data=subset(d4,Date<as.Date("2020-01-01")),aes(y=estimated_sum, colour = "Actual"), size=1.2) +
  geom_line(data=subset(d4,Date>=as.Date("2020-01-01")),aes(y=estimated_sum, colour = "Estimate"), size=1.2, linetype=1) +
  theme_bw() + theme(legend.title = element_blank()) + ylab("") + xlab("") +
  geom_vline(xintercept=as.numeric(as.Date("2020-01-01")), linetype=2) + 
  geom_ribbon(data=subset(d4,Date>=as.Date("2020-01-01")),aes(ymin=LL_sum, ymax=UL_sum, fill="95% conf."), alpha=0.5) +
  scale_fill_manual("", values=c("95% conf."="grey")) +
  theme(axis.text.x=element_text(angle = -90, hjust = 0))+
  labs(title="Cumulative Maryland ZEV sales from 2011-2019, forecast to 2025")
